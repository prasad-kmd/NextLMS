...
export default function RegisterPage() {
  ...
  const registerUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Registered!"); router.push("/login")
    } catch (error: any) {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <Card className="w-[400px]">
        <CardHeader><CardTitle className="text-2xl text-center">Register</CardTitle></CardHeader>
        <form onSubmit={registerUser}>
          <CardContent className="space-y-4">
            <div className="space-y-2"><label>Name</label><Input value={data.name} onChange={(e) => setData({ ...data, name: e.target.value })} required /></div>
            ...
            <div className="space-y-2"><label>Role</label>
              <Select value={data.role} onValueChange={(v) => setData({ ...data, role: v || "STUDENT" })}>
          ...
          <CardFooter className="flex flex-col space-y-4">
            ...
            <p className="text-sm">Have an account? <Link href="/login" className="text-blue-500">Login</Link></p>
          </CardFooter>
        </form>
      </Card>
    </div>
