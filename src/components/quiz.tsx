...
export function Quiz({ id, title, questions }: QuizProps) {
  ...
  const handleNextQuestion = async () => {
    ...
    if (currentQuestion + 1 < questions.length) {
      ...
      if (id) {
        ...
        try {
          const result = await submitQuizAttempt(id, newAnswers)
  ...
  if (showResult) {
    const displayScore = score !== null ? score : answers.length
        ...
        <CardHeader className="bg-primary/5 text-center pb-8 pt-8 border-b">
          ...
          <div className="mt-4 flex flex-col items-center">
            <div className="relative h-24 w-24 flex items-center justify-center mb-4">
               <svg className="h-full w-full transform -rotate-90">
                ...
                <circle
                  ...
                  strokeDashoffset={2 * Math.PI * 40 * (1 - displayScore / questions.length)}
            ...
            <p className="text-muted-foreground font-google-sans">
              {displayScore === questions.length ? "Perfect score! You're an expert." :
               displayScore >= questions.length / 2 ? "Great job! You have a good understanding." :
    ...
    <Card className="my-8 overflow-hidden border-border bg-background/50 backdrop-blur-sm shadow-lg interactive-quiz-card">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isAnswered && !isSelected && "opacity-40 grayscale-[0.5]",
      ...
      <CardFooter className="border-t bg-muted/5 py-4 justify-between">
        {!isAnswered ? (
          ...
          >
            {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : (currentQuestion + 1 < questions.length ? "Next Question" : "See Results")}
