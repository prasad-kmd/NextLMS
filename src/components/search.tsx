...
import { Search as SearchIcon, X, Calendar, ArrowRight, Loader2, Home, LayoutDashboard, BookOpen, FileText } from "lucide-react"
...
export function Search({ isMobileSidebar = false }: SearchProps) {
    ...
    const handleResultClick = (type: string, slug: string) => {
        const path = type === 'course' ? `/courses/${slug}` : type === 'lecture' ? `/courses/lecture/${slug}` : `/${type}/${slug}`
        router.push(path)
        handleClose()
    }
